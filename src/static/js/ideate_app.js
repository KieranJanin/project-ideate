...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (currentSimulationState.collectedData.ideas) {
        renderers.renderAnalogousBrainstorm(currentSimulationState.collectedData.ideas);
    }
    if (currentSimulationState.collectedData.evaluatedIdeas) {
        renderers.renderEffortImpactMatrix(currentSimulationState.collectedData.evaluatedIdeas);
    }
    if (currentSimulationState.collectedData.winningConcept) {
        renderers.renderWinningConcept(currentSimulationState.collectedData.winningConcept);
    }

    const phaseIndex = phases.indexOf('Ideate');
    let startIndex = 0;
    ...
    if (!currentSimulationState.isPaused) {
        if (currentSimulationState.scriptIndex < startIndex || currentSimulationState.currentPhase !== 'ideate') {
            currentSimulationState.scriptIndex = startIndex;
            currentSimulationState.currentPhase = 'ideate';
